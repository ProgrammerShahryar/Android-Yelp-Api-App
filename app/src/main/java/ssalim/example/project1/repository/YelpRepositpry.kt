...
class YelpRepository(private val retrofit: Retrofit) {
    ...
    suspend fun searchBusinesses(location: String, businessesLiveData: MutableLiveData<List<YelpData>>) {
        val yelpService = retrofit.create(YelpService::class.java)
        try {
            val response = yelpService.searchBusinesses("Bearer $apiKey", "restaurants", location)
